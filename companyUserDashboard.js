...
makeRequest("/companyUserData", function (data) {
  ...
  am4core.ready(function () {
    ...
    for (let i = data1.feeds.length - 1; i > data1.feeds.length - 12; i--) {
      ...
      //console.log(time);
      var h = parseInt(data1.feeds[i].created_at.substring(11, 13));
      var m = parseInt(data1.feeds[i].created_at.substring(14, 16));
      if (m + 30 > 60) {
        h = (h + 6) % 24;
        m = (m + 30) % 60;
      } else {
      ...
      if (h < 10) h = "0" + h;
      if (m < 10) m = "0" + m;
      var rest = h + ":" + m + data1.feeds[i].created_at.substring(16, 19);
      cum +=
        "<tr>" +
        "<td>" +
